...
import {
  ...
  ProductCategoryList,
  UrlParameters,
} from "../../model/product-configuration.model";
...
export class ProductConfigurationService {
  ...
  /**
   * Get a single product category information
   *
   ...
   * @returns
   */
  getProductCategoryList(parameters: UrlParameters) {
    // Prepare parameters
    let params = new HttpParams({ fromObject: parameters as any });

    // Set the url to get information
    let url = this.apiURL + "product-category/list";

    return this.http.get<ProductCategoryList[]>(url, { params }).pipe(
      // Map data to change date format
      map((data: any) => {
        // Update date
        const updatedList = data.list.map((category: ProductCategoryList) => {
          return {
            ...category,
   ...
   * @returns Observable request
   */
  updateProductCategory(categoryId: number, modifiedCategory: any) {
    // Set the url
    const url = `${this.apiURL}product-category/${categoryId}`;
